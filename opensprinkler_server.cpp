...
void server_change_runonce(OTF_PARAMS_DEF) {
	...
	for(unsigned char oi=0;oi<ns;oi++) {
		...
		if (dur>0 && !(os.attrib_dis[bid]&(1<<s))) {
			// Use schedule_station_with_fertigation for run-once to support fertigation
			RuntimeQueueStruct *q = schedule_station_with_fertigation(sid, water_time_resolve(dur), 254, &prog);
			if (q) {
				match_found = true;
			}
		}
